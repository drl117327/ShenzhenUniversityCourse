...
if __name__ == "__main__":
	...
	activate = ["relu", "tanh", "logistic"]
	results = []

	for size in activate:
		model = MLPClassifier(
		...
		plt.plot(
			...
			loss_curve,
			label=f"activation={size}",
		)

	plt.xlabel("Epoch")
	plt.ylabel("Loss")
	plt.title("MLP Training Loss Curves (Different Activations)")
	plt.grid(True, linestyle="--", alpha=0.5)
	plt.legend()
	plt.tight_layout()
	plt.savefig("./output/2024150040邓瑞霖MLP_loss_compare2.png")
